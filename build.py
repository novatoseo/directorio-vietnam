...
def seo_title_center(center):
    """Genera un title corto para fichas de centro (objetivo <= ~60 chars)."""
    name = center['name']
    city = center.get('city', 'Việt Nam')
    base = f'{name} tại {city}'
    if len(base) > 52:
        base = trunc(base, 52)
    return f'{base} | Anh Ngữ VN'


def build_unique_value_block(center):
    """Genera un bloque de contenido único por ficha — distrito, perfil objetivo,
    cursos detectados y enlaces internos contextuales — para diferenciar el contenido
    programático y reducir el riesgo de thin content."""
    city = center.get('city', 'Việt Nam')
    city_slug = center.get('city_slug', '')
    district = center.get('district', '')
    category = center.get('category_primary', 'trung tâm Anh ngữ')
    category_slug = center.get('category_slug', '')
    services = center.get('services', []) or []
    name = center['name']

    # Detectar tipo de cursos por palabras clave en services
    services_text = ' '.join(services).lower() if services else ''
    targets = []
    if any(k in services_text for k in ['ielts', 'toefl']):
        targets.append('người chuẩn bị thi <strong>IELTS / TOEFL</strong> để du học hoặc xin việc quốc tế')
    if 'toeic' in services_text:
        targets.append('người đi làm cần chứng chỉ <strong>TOEIC</strong>')
    if any(k in services_text for k in ['kid', 'tre em', 'trẻ', 'children', 'mầm']):
        targets.append('phụ huynh tìm chương trình <strong>tiếng Anh trẻ em</strong>')
    if any(k in services_text for k in ['business', 'doanh nghiệp', 'kinh doanh']):
        targets.append('người đi làm cần <strong>tiếng Anh thương mại</strong>')
    if any(k in services_text for k in ['giao tiếp', 'communication', 'speaking']):
        targets.append('học viên muốn cải thiện <strong>kỹ năng giao tiếp hằng ngày</strong>')

    if not targets:
        targets.append(f'người đang tìm một {category.lower()} đáng tin cậy tại {city}')

    targets_html = '<ul class="profile-list">' + ''.join(f'<li>{t}</li>' for t in targets[:4]) + '</ul>'

    # Servicios destacados
    services_html = ''
    if services:
        top_services = services[:4]
        srv_items = ''.join(f'<li>{esc(s)}</li>' for s in top_services)
        services_html = f'<h3>Chương trình chính</h3><ul class="services-inline">{srv_items}</ul>'

    # Contexto local
    district_text = ''
    if district:
        district_text = f' — cụ thể tại khu vực <strong>{esc(district)}</strong>'

    # Enlaces internos contextuales
    internal_links = []
    if city_slug:
        internal_links.append(f'<a href="../thanh-pho/{city_slug}.html">Xem các trung tâm khác tại {esc(city)}</a>')
    if category_slug:
        internal_links.append(f'<a href="../danh-muc/{category_slug}.html">So sánh với {esc(category.lower())} khác</a>')
    links_html = ''
    if internal_links:
        links_html = '<div class="internal-links-block"><p><strong>Tiếp tục khám phá:</strong></p>' + \
                     ''.join(f'<p>→ {l}</p>' for l in internal_links) + '</div>'

    return f'''<div class="detail-section unique-block">
      <h2 class="detail-h"><span class="icon" aria-hidden="true">✨</span>Ai phù hợp với {esc(name)}?</h2>
      <p>Dựa trên thông tin công khai, {esc(name)} là một lựa chọn đáng cân nhắc cho:</p>
      {targets_html}
      <p>Trung tâm đặt tại {esc(city)}{district_text}. Khi so sánh các lựa chọn, hãy cân nhắc vị trí thực tế, lịch học linh hoạt, đánh giá từ học viên cũ và mục tiêu cá nhân của bạn — giao tiếp, IELTS, TOEIC, tiếng Anh trẻ em hay tiếng Anh chuyên ngành.</p>
      {services_html}
      {links_html}
    </div>'''
...
def build_ficha(center, data):
    ...
    # Bloque de contenido único — diferencia el contenido programático
    sections.append(build_unique_value_block(center))
...
def build_category(cat, data):
    ...
    head = meta_head(title, description, canonical,
                     noindex=(len(centers_in_cat) < 3)).replace('__CSSPATH__','../')
...
def build_home(data):
    ...
    # Schema WebSite + Organization (sin SearchAction, no implementada)
    schemas = [
        {
